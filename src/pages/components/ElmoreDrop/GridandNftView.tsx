...
import styles from '../styles/ElmoreDrop/GridandNftView.module.scss';
import {
  NFTbg,
  noImage165by165,
...
const GridandNftView = ({ nftsData, autoRerender, GridCss }: dropCardProps) => {
  ...
  const isInfinite = location.pathname === '/drop/infinite-worlds';
  const isExploringFantasy = location.pathname === '/drop/exploring-fantasy';
  const isValentine = location.pathname === '/drop/valentine';
  ...
  return (
    <>
      ...
      <div className={nftsData?.length >= 3 ? styles.wrap2 : styles.wrap}>
        ...
        <div
          ref={scrollDiv}
          id="3d-display"
          className={`${styles.NFTWrap2} ${videoId !== '' && styles.NFT3d}`}
